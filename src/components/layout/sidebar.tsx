...
export const BOTTOM_NAV_ITEMS = [
  { icon: Settings, label: "Settings", path: "/profile/update" },
...
export function Sidebar() {
  ...
  return (
    <aside className="hidden md:flex flex-col w-[260px] border-r border-sidebar-border h-dvh sticky top-0 bg-sidebar z-40 px-4 py-6">
      ...
      {/* App Logo */}
      <Link href="/" className="flex items-center mt-[-20px] gap-1.5 px-10 mb-2 transition-opacity hover:opacity-80">
        <span className="font-display-lg text-[38px] font-black tracking-tighter text-primary">
          Closy
        </span>
      </Link>
        ...
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <Link
              ...
              href={item.path}
              className={cn(
                "flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all font-body-sm text-[14px]",
                isActive
      ...
      {/* Bottom Navigation */}
      <div className={cn("border-t border-sidebar-border pt-3 flex flex-col gap-0.5", user?.isPremium && "mt-auto")}>
        ...
        <button
          ...
          className="flex w-full items-center gap-3 px-3 py-2.5 rounded-xl transition-all font-body-sm text-[14px] text-red-500/80 hover:text-red-500 hover:bg-red-500/10 mt-1"
        >
