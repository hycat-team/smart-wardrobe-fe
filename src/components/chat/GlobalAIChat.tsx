...
import { Bot, X, MoveRight, Loader2 } from "lucide-react";
import { cn } from "@/lib/utils";
import gsap from "gsap";
import { useGSAP } from "@gsap/react";
import { useRouter } from "next/navigation";
import { aiApi } from "@/features/ai-stylist/api/ai.api";
import { ChatMessage, ChatSessionRes } from "@/features/ai-stylist/types";
...
export function GlobalAIChat() {
  ...
  const handleRedirectToGenerate = (details?: string) => {
    setIsOpen(false);
    if (details) {
      // const searchParams = new URLSearchParams();
      // searchParams.set('details', details);
      // router.push(`/ai-stylist?${searchParams.toString()}`);
  ...
  return (
    <div ref={containerRef} className="fixed bottom-24 md:bottom-6 right-4 md:right-6 z-[100] flex flex-col items-end justify-end pointer-events-none gap-4">
      ...
      >
        ...
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-[#E5E5E5] bg-[#F9F9F9]">
          <div className="flex items-center gap-2">
            <Bot className="w-5 h-5 text-[#1A1A1A]" />
            <h3 className="font-bold text-[13px] text-[#1A1A1A] uppercase tracking-widest">CLOSY AI STYLIST</h3>
          </div>
          <button onClick={() => setIsOpen(false)} className="text-[#A3A3A3] hover:text-[#1A1A1A] transition-colors p-1">
            <X className="w-4 h-4" />
          </button>
        </div>
          ...
          {chatMessages.length === 0 ? (
            ...
            chatMessages.map((msg) => (
              <AIChatMessage
                ...
                isGenerating={false}
                onGenerateRedirect={handleRedirectToGenerate}
        ...
        {/* Input Area */}
        <div className="p-3 border-t border-[#E5E5E5] bg-white relative flex flex-col">
          <div className="relative border border-[#E5E5E5] rounded-xl overflow-hidden focus-within:border-[#1A1A1A] transition-colors bg-[#F9F9F9]">
            <input
              value={chatInput}
              onChange={(e) => setChatInput(e.target.value)}
              onKeyDown={handleKeyDown}
              disabled={isChatting}
              className="w-full bg-transparent border-none outline-none px-3 py-3 text-[13px] text-[#1A1A1A] placeholder:text-[#A3A3A3] disabled:opacity-50"
              placeholder="Hỏi AI tư vấn phối đồ..."
              type="text"
              maxLength={2000}
            />
            <button
              ...
              className="absolute right-1 top-1/2 -translate-y-1/2 w-8 h-8 flex items-center justify-center bg-[#1A1A1A] text-white rounded-lg hover:bg-black transition-colors disabled:opacity-50"
            >
          ...
          >
            {isHistoryOpen ? "Đóng Lịch sử" : "Xem Lịch sử Chat"}
          </button>
        </div>
      </div>
      ...
      <button
        ...
        className="w-14 h-14 bg-[#1A1A1A] hover:bg-black text-white rounded-full flex items-center justify-center shadow-xl hover:shadow-2xl hover:scale-105 transition-all pointer-events-auto shrink-0"
      >
