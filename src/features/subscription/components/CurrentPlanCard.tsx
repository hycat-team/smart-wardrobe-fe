      ...
      <div className="flex flex-col md:flex-row justify-between gap-8 md:gap-12 relative z-10">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-4 mb-6">
            <h2 className="font-semibold text-4xl md:text-5xl font-medium tracking-tight text-white">
              {subscription.planName || subscription.PlanName || 'Gói Premium'}
            </h2>
            <span className="px-3 py-1 bg-white/10 text-white text-[10px] font-semibold font-bold uppercase tracking-widest border border-white/20">
              {subscription.status || 'ACTIVE'}
            </span>
          </div>

          <div className="space-y-6 mt-10">
            <div className="flex items-start gap-4 text-white/70 font-semibold text-[12px] uppercase tracking-wider">
            ...
            <div className="flex items-center gap-4 text-white/70 font-semibold text-[12px] uppercase tracking-wider">
              ...
              <div className="flex items-center justify-between w-full max-w-[280px]">
                ...
                <button
                  ...
                  className={`relative inline-flex h-5 w-9 flex-shrink-0 cursor-pointer rounded-none border border-transparent transition-colors duration-200 ease-in-out focus:outline-none focus:ring-1 focus:ring-[#D9C5B2] focus:ring-offset-1 focus:ring-offset-[#1A1A1A] ${autoRenew ? 'bg-[#D9C5B2]' : 'bg-white/20'
                    }`}
                >
                  <span
                    aria-hidden="true"
                    className={`pointer-events-none inline-block h-4 w-4 transform bg-[#1A1A1A] transition duration-200 ease-in-out ${autoRenew ? 'translate-x-4' : 'translate-x-0'
                      }`}
                  />
                </button>
              </div>
        ...
        <div className="flex-1 bg-white/5 p-6 md:p-8 border border-white/10">
          <h3 className="font-semibold text-[11px] font-bold text-[#D9C5B2] uppercase tracking-[0.2em] mb-8 border-b border-white/10 pb-4">
            HẠN NGẠCH HÔM NAY
          </h3>
          ...
          <div className="space-y-8">
            {/* Outfits Quota */}
            <div>
              <div className="flex justify-between font-semibold text-[11px] uppercase tracking-wider mb-3">
              ...
              {(quota.outfitRecommendCount || quota.OutfitRecommendCount || 0) === 0 && (
                <p className="text-[9px] text-white/40 mt-2 italic font-semibold uppercase">Chưa sử dụng</p>
              )}
            </div>
