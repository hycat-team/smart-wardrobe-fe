...
export function useGhostCloset() {
  ...
  const saveItem = (itemId: string) => {
    ...
    if (typeof window !== "undefined")
      localStorage.setItem("closy_ghost_saved", JSON.stringify(updated));
  };
  ...
  const joinWaitlist = (itemId: string) => {
    ...
    if (typeof window !== "undefined")
      localStorage.setItem("closy_ghost_waitlist", JSON.stringify(updated));
  };
  ...
  const hideBrand = (brandId: string) => {
    ...
    if (typeof window !== "undefined")
      localStorage.setItem("closy_ghost_hidden_brands", JSON.stringify(updated));
  };

  const logGhostAction = (
    sampleId: string,
    action: "save" | "waitlist" | "notMyStyle" | "swap" | "keep"
  ) => {
