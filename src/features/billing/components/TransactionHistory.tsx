  ...
  return (
    <div className="bg-zinc-900/50 backdrop-blur-xl border border-white/5 rounded-3xl p-6 md:p-8 shadow-2xl">
      ...
      <div className="space-y-4">
        ...
        {statements.length > 0 ? (
          statements.map((stmt) => {
            const isPositive = stmt.type === "TOPUP" || stmt.type === "REFUND";
            return (
              <div
                key={stmt.id}
                className="flex items-center justify-between p-5 rounded-2xl bg-zinc-950/50 border border-zinc-800/50 hover:bg-zinc-900 hover:border-zinc-700 transition-colors"
              >
                {" "}
                <div className="flex items-center gap-4">
                  {" "}
                  <div
                    className={`w-12 h-12 rounded-full flex items-center justify-center ${isPositive ? "bg-green-500/10 text-green-400" : "bg-red-500/10 text-red-400"}`}
                  >
                    {" "}
                  ...
                  <div>
                    ...
                    <h4 className="text-zinc-200 font-medium mb-1">
                      {stmt.description || (stmt.type === "TOPUP" ? "Nạp tiền" : "Thanh toán gói")}
                    </h4>{" "}
                    <span className="text-xs text-zinc-500">
                      {" "}
                      {new Date(stmt.createdAt).toLocaleString("vi-VN")}{" "}
                    </span>{" "}
                  </div>{" "}
                </div>{" "}
                <div className="text-right">
                  ...
                  >
                    {" "}
                    {isPositive ? "+" : "-"}
                    {stmt.amount.toLocaleString("vi-VN")}đ{" "}
                  </div>{" "}
                  <div className="text-xs text-zinc-500 mt-1">
                    {" "}
                    Số dư: {stmt.balanceAfter.toLocaleString("vi-VN")}đ{" "}
                  </div>{" "}
                </div>{" "}
              </div>
            );
