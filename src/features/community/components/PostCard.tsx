      ...
      <article className="w-full flex flex-col bg-white border border-black/5 rounded-2xl shadow-sm hover:shadow-md transition-shadow group overflow-hidden">
        ...
        <div className="p-5 flex items-center justify-between border-b border-black/5 bg-[#FAFAFA]/50">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex flex-col">
              {" "}
              <span className="font-bold text-sm text-[#111] font-sans leading-tight hover:underline">
                {" "}
                {post.username || "ATELIER CURATORS"}{" "}
              </span>{" "}
              {/* <span className="text-[10px] font-medium text-[#A3A3A3] mt-0.5"> USER </span> */}{" "}
            </div>{" "}
          </div>{" "}
          <div className="flex items-center gap-4">
            {" "}
        ...
        {hasMedia && mediaUrl && (
          <div className="relative w-full aspect-[4/5] bg-[#F5F2EE] overflow-hidden">
            ...
            {!isImageLoaded && !hasImageError && (
              <Skeleton className="absolute inset-0 w-full h-full rounded-xl bg-gray-100" />
            )}{" "}
            {hasImageError ? (
        ...
        <div className="p-6 flex flex-col gap-5 bg-white">
            ...
            <div className="flex items-center gap-6">
              ...
              >
                ...
                <Heart
                  ref={heartIconRef}
                  className={cn(
                    "w-6 h-6 transition-colors hover:text-[#A3A3A3]",
                    post.isLiked ? "fill-[#D03027] text-[#D03027] hover:text-[#D03027]" : ""
                  )}
                  strokeWidth={1.5}
                />{" "}
              </button>{" "}
          ...
          <div className="text-sm text-[#111] leading-relaxed">
            {" "}
            {/* {post.title && ( <span className="font-bold font-sans mr-3 block mb-1">{post.title}</span> )} */}{" "}
            <span className="font-bold font-sans mr-3">{post.username || "ATELIER CURATORS"}</span>{" "}
