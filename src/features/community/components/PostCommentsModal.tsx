  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent aria-describedby={undefined} className="w-[95vw] sm:max-w-[1200px] md:w-[85vw] bg-background border border-border text-foreground rounded-3xl p-0 overflow-hidden flex flex-col md:flex-row h-[80vh] shadow-xl [&>button]:hidden">
        ...
        {/* Left Side: Post Visual Media */}
        <div className="hidden md:flex w-[60%] h-full bg-black/5 dark:bg-black/40 relative border-r border-border items-center justify-center overflow-hidden">
          {isOutfit && outfitCover ? (
            <div className="relative w-full h-full">
              <Image 
                fill 
                sizes="(max-width: 1200px) 50vw, 60vw" 
                src={outfitCover}
                alt={post.outfit?.name || post.title || 'Outfit'}
            ...
            <div className="w-full h-full flex items-center justify-center p-12 bg-muted text-foreground">
              <div className="flex flex-col gap-4 text-center max-w-md">
                {post.title && <h2 className="font-bold text-2xl">{post.title}</h2>}
        ...
        <div className="w-full md:w-[40%] flex flex-col h-full bg-background relative">
          ...
          <div className="p-6 border-b border-border flex flex-col gap-4 bg-muted/20">
              ...
              <div className="flex items-center gap-3">
                <Link href={`/users/${authorUsername}`} className="shrink-0">
                  <Image
                    ...
                    width={40}
                    height={40}
                    className="w-10 h-10 rounded-full object-cover ring-1 ring-border shrink-0 hover:opacity-90 transition-opacity"
                  />
                </Link>
                <div className="flex flex-col">
                  <Link href={`/users/${authorUsername}`} className="hover:underline">
                    <span className="font-bold text-sm text-foreground">@{authorUsername}</span>
                  </Link>
                  <span className="text-xs text-muted-foreground font-medium">{authorDisplayName}</span>
                </div>
              </div>
              <button 
                onClick={onClose} 
            ...
            <div className="text-sm text-foreground leading-relaxed max-h-32 overflow-y-auto pr-1">
              {post.title && <span className="font-bold mr-1.5">{post.title}.</span>}
              {post.content}
            </div>
          </div>
          ...
          <div className="p-4 border-t border-border bg-background mt-auto flex flex-col gap-2">
            {replyingTo && (
              <div className="flex items-center justify-between text-[11px] font-medium text-muted-foreground px-2 py-1 bg-muted rounded-full">
                <span>Đang trả lời <span className="font-bold text-foreground">@{replyingTo.username}</span></span>
                <button
            ...
            <form onSubmit={handleAddComment} className="flex items-center gap-2">
              ...
              >
                {isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : 'Đăng'}
              </button>
            </form>
          </div>
        </div>
