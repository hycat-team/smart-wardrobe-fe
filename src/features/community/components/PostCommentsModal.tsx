...
export const PostCommentsModal = ({ isOpen, onClose, post }: PostCommentsModalProps) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      {" "}
      <DialogContent className="w-[95vw] sm:max-w-[1000px] md:w-[85vw] bg-white border border-[#E5E5E5] text-[#1A1A1A] rounded-xl p-0 overflow-hidden flex flex-col md:flex-row h-[80vh] shadow-xl [&>button]:hidden">
        {" "}
        ...
        <div className="w-full md:w-[40%] flex flex-col h-full bg-white relative">
          ...
          <div className="p-6 border-b border-[#E5E5E5] flex flex-col gap-4">
            ...
            <div className="flex justify-between items-start">
              ...
              <button
                onClick={onClose}
                className="text-[#A3A3A3] hover:text-[#1A1A1A] transition-colors p-1"
              >
                {" "}
                <X className="w-5 h-5" />{" "}
              </button>{" "}
          ...
          <div className="p-4 border-t border-[#E5E5E5] bg-white mt-auto flex flex-col gap-2">
            ...
            {replyingTo && (
              <div className="flex items-center justify-between text-[11px] font-medium text-[#666666] px-2 py-1 bg-[#F5F2EE] rounded-sm">
                ...
                <span>
                  Replying to{" "}
                  <span className="font-bold text-[#1A1A1A]">@{replyingTo.username}</span>
                </span>{" "}
                <button
            ...
            <form onSubmit={handleAddComment} className="flex items-center gap-2">
              ...
              <button
                ...
                className="text-xs font-bold text-[#1A1A1A] hover:text-black disabled:opacity-30 disabled:hover:text-[#1A1A1A] transition-colors px-2 py-1"
              >
                {" "}
