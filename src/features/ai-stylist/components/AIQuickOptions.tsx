  ...
  return (
    <Popover open={popoverOpen} onOpenChange={setPopoverOpen}>
      ...
      <PopoverContent
        side="top"
        align="start"
        className="w-[320px] max-h-[400px] overflow-y-auto p-5 border-[#E5E5E5] rounded-xl shadow-xl bg-white"
      >
        {" "}
        <div className="flex flex-col gap-6">
          ...
          <div className="border-b border-[#E5E5E5] pb-2">
            {" "}
            <p className="text-[11px] font-bold text-[#1A1A1A] ">THÔNG SỐ NHANH</p>{" "}
          </div>{" "}
          <div className="flex flex-col gap-3">
            {" "}
            <p className="text-[9px] text-[#888888] font-bold ">DỊP (OCCASION)</p>{" "}
            <div className="flex flex-wrap gap-1.5">
              ...
              {OCCASIONS.map((occ) => (
                <button
                  ...
                  className={cn(
                    "px-2 py-1 text-[9px] font-bold transition-colors border",
          ...
          <div className="flex flex-col gap-3">
            {" "}
            <p className="text-[9px] text-[#888888] font-bold ">PHONG CÁCH (STYLE)</p>{" "}
            <div className="flex flex-wrap gap-1.5">
              {" "}
              {STYLES.map((style) => (
                <button
          ...
          <div className="flex flex-col gap-3">
            {" "}
            <p className="text-[9px] text-[#888888] font-bold ">MÙA (SEASON)</p>{" "}
            <div className="flex flex-wrap gap-1.5">
              {" "}
              {SEASONS.map((s) => (
                <button
          ...
          <div className="flex flex-col gap-3">
            {" "}
            <p className="text-[9px] text-[#888888] font-bold ">THỜI TIẾT (WEATHER)</p>{" "}
            <div className="flex flex-wrap gap-1.5">
              {" "}
              {WEATHERS.map((w) => (
                <button
                  key={w}
                  onClick={() => setSelectedWeather((prev) => (prev === w ? "" : w))}
                  className={cn(
                    "px-2 py-1 text-[9px] font-bold transition-colors border",
                    selectedWeather === w
                      ? "bg-black text-white border-black"
          ...
          <div className="flex flex-col gap-3">
            {" "}
            <p className="text-[9px] text-[#888888] font-bold ">TÔNG MÀU (COLOR TONE)</p>{" "}
            <div className="flex flex-wrap gap-1.5">
              {" "}
              {COLOR_TONES.map((c) => (
                <button
                  key={c}
                  onClick={() => setSelectedColorTone((prev) => (prev === c ? "" : c))}
                  className={cn(
                    "px-2 py-1 text-[9px] font-bold transition-colors border",
                    selectedColorTone === c
                      ? "bg-black text-white border-black"
          ...
          <button
            ...
            className="mt-2 w-full bg-[#1A1A1A] text-white text-[11px] font-bold py-3.5 hover:bg-black/80 transition-colors flex justify-center items-center gap-2 disabled:opacity-50 "
          >
            {" "}
