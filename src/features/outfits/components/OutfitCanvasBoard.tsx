...
export function OutfitCanvasBoard({
  ...
  return (
    <div className="flex-1 bg-[#F9F9F9] border border-[#E5E5E5] relative overflow-hidden flex items-center justify-center">
        ...
        {selectedItems.length > 0
          ? selectedItems.map((item) => {
              const hasAlternatives =
                item._role && hasAlternativesCheck ? hasAlternativesCheck(item._role) : false;
              return (
                <motion.div
                  layout
                  ...
                  <div className="relative">
                    ...
                    <div className="canvas-item-controls opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto transition-opacity absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-white border border-[#1A1A1A] text-[#1A1A1A] flex items-center gap-0 shadow-sm z-50 whitespace-nowrap">
                      ...
                      <button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          updateScale(item.id, item.scale - 10);
                        }}
                      ...
                      <button
                        ...
                        className="hover:bg-[#1A1A1A] hover:text-white transition-colors flex items-center gap-1 text-[9px] font-bold px-3 py-2"
                        title="Đưa lên trên"
                      >
                        {" "}
                      ...
                      {item.isGhost && onGhostItemClick && (
                        <>
                          ...
                          <button
                            ...
                            className="hover:bg-[#A0522D] hover:text-white text-[#A0522D] transition-colors flex items-center gap-1 text-[9px] font-bold px-3 py-2"
                    ...
                    <div
                      ...
                      style={{ width: `${item.scale * 2.2}px`, height: "auto" }}
                    >
                      {" "}
                      <img
                      ...
                      />{" "}
                      {item.isGhost && (
                        <GhostItemBadge brandName={item.brandName || "Local Brand"} />
                      )}{" "}
                    </div>{" "}
      ...
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex items-center gap-2 text-[10px] font-bold text-[#888888] bg-white px-4 py-2 border border-[#E5E5E5] z-20 pointer-events-none shadow-sm whitespace-nowrap ">
        {" "}
