  ...
  if (cart.length === 0) {
    return (
      <div className="flex-1 bg-white text-black min-h-[70vh] flex flex-col items-center justify-center">
        ...
        <Link href="/community">
          <Button className="rounded-none bg-black text-white font-bold uppercase tracking-widest px-8 h-12">
            Khám phá Brands
          </Button>
        </Link>
      </div>
      ...
      <div className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-8 flex flex-col gap-6">
            <div className="hidden sm:grid grid-cols-12 gap-4 pb-4 border-b border-black/10 text-xs font-bold uppercase tracking-widest text-black/50">
            ...
            <div className="flex flex-col gap-6">
              {cart.map((item, idx) => {
                ...
                return (
                  <div key={idx} className="grid grid-cols-1 sm:grid-cols-12 gap-4 items-center py-4 sm:py-0 border-b border-black/10 sm:border-0">
                    <div className="col-span-1 sm:col-span-6 flex gap-4">
                      ...
                      <div className="flex flex-col justify-center gap-1">
                        <Link href={`/brands/${item.brandId}`} className="text-xs font-bold text-black/50 hover:text-black uppercase tracking-wider">
                          {brand?.name}
                        </Link>
                        <Link href={`/products/${item.productId}`} className="font-bold text-sm hover:underline">
                          {item.name}
                        </Link>
                        <div className="text-xs text-black/60 mt-1">
                          Size: <span className="font-bold text-black">{item.size}</span> | Màu: <span className="font-bold text-black">{item.color}</span>
                        </div>
                        <div className="text-sm font-bold mt-2 sm:hidden">{item.price.toLocaleString()}đ</div>
                      </div>
                    </div>
          ...
          <div className="lg:col-span-4">
            <div className="bg-[#FAFAFA] p-6 lg:p-8 flex flex-col gap-6 sticky top-10">
              <h2 className="font-bold text-lg uppercase tracking-widest border-b border-black/10 pb-4">Tóm tắt đơn hàng</h2>
              ...
              <Link href="/checkout" className="w-full mt-4">
                <Button className="w-full rounded-none bg-black hover:bg-black/90 text-white h-14 font-bold uppercase tracking-widest flex items-center justify-center gap-2 group">
