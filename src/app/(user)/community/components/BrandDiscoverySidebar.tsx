    ...
    <div className="flex flex-col gap-5 border border-black/10 p-6 bg-[#FAFAFA]">
      ...
      <div className="relative">
        ...
        <Input 
          ...
          className="pl-9 border-black/20 focus-visible:ring-black rounded-none h-10"
        />
      </div>
            ...
            <div key={brand.id} className="flex items-center justify-between group">
              <Link href={`/brands/${brand.id}`} className="flex items-center gap-3">
                ...
                <div className="flex flex-col">
                  <span className="font-bold text-sm text-black group-hover:underline decoration-1 underline-offset-2">{brand.name}</span>
                  <span className="text-xs text-black/60 font-medium truncate max-w-[120px]">{brand.styles.join(', ')}</span>
                </div>
              </Link>
            </div>
          ))
      ...
      <div className="pt-2 border-t border-black/10">
        <Link href="/brands">
          <button className="text-[10px] font-bold uppercase tracking-widest text-black/60 hover:text-black transition-colors w-full text-center">
            Xem tất cả Brands
          </button>
        </Link>
      </div>
