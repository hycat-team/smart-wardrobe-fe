...
export default function PostDetailClient({ postPublicId, initialData }: PostDetailClientProps) {
  ...
  if (!post) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-4">
        {" "}
        <p className="text-sm text-[#111] font-sans">Không tìm thấy bài viết.</p>{" "}
        <button
          onClick={() => router.push("/community")}
          className="text-xs border-b border-black/20 pb-0.5 hover:border-black transition-colors"
        >
          {" "}
          Quay lại cộng đồng{" "}
        </button>{" "}
  ...
  return (
    <div className="max-w-2xl mx-auto py-8 px-4">
      ...
      <button
        onClick={() => router.push("/community")}
        className="flex items-center gap-2 text-sm text-[#666] hover:text-[#111] transition-colors mb-6 font-sans group outline-none"
      >
        {" "}
