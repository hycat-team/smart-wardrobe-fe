...
export function ProfileUpdateClient({ initialProfile }: { initialProfile: UserRes }) {
  ...
  const inputClassName =
    "w-full rounded-xl border border-cream-dark/50 bg-cream-dark/30 px-4 py-3 font-body-sm text-ink outline-none transition-all placeholder:text-ink-muted/50 focus:border-ink focus:bg-transparent focus:ring-1 focus:ring-ink";
    ...
    <div className="mx-auto w-full max-w-2xl animate-in fade-in pb-20 pt-10 duration-700">
      <div className="mb-10 space-y-3 text-center">
        ...
        <p className="font-body-sm text-ink-muted">
          C?p nh?t h? so v� nh?ng t�y ch?n ph� h?p v?i b?n.
        </p>
      </div>
      ...
      <div className="relative mb-12 flex justify-center">
        <div className="group relative size-24 cursor-not-allowed overflow-hidden rounded-full border border-cream-dark/50 bg-cream-dark/30">
          ...
          <div className="absolute inset-0 flex items-center justify-center bg-black/10 opacity-0 transition-opacity group-hover:opacity-100">
            <span className="rounded bg-white/80 px-2 py-1 text-xs font-medium text-ink">
              Ch?nh s?a
            </span>
          </div>
        </div>
      ...
      <form onSubmit={handleSaveProfile} className="space-y-8">
        <div className="space-y-6">
          <div className="grid gap-6 md:grid-cols-2">
            <div>
              <label className={labelClassName}>T�n</label>
              <input
                name="firstName"
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <div>
              <label className={labelClassName}>Ng�y sinh</label>
              <div
                className={cn(
                  "flex w-full items-center overflow-hidden p-0 pr-3 transition-all",
                  inputClassName,
                  !profileData.dateOfBirth && "text-ink-muted/50"
                )}
              >
                <input
                  type="date"
            ...
            <div>
              <label className={labelClassName}>Gi?i t�nh</label>
              <Select
                value={profileData.gender ? profileData.gender.toString() : ""}
                onValueChange={(value) =>
                  setProfileData((prev) => ({ ...prev, gender: Number(value) }))
                }
              >
                <SelectTrigger
                  style={{ width: "100%", height: "68%" }}
                  className={cn("w-full items-center justify-between", inputClassName)}
                >
                  <SelectValue placeholder="Ch?n gi?i t�nh">
                    {profileData.gender === Gender.Male && "Nam"}
                    {profileData.gender === Gender.Female && "N?"}
                    {profileData.gender === Gender.Other && "Kh�c"}
                  </SelectValue>
                </SelectTrigger>
                <SelectContent
                  alignItemWithTrigger={false}
                  className="z-50 rounded-xl border-cream-dark/50 bg-background shadow-xl"
                >
                  <SelectItem
                    value={Gender.Male.toString()}
                    className="cursor-pointer focus:bg-cream-dark/30"
                  >
                    Nam
                  </SelectItem>
                  <SelectItem
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <div>
              <label className={labelClassName}>�?a ch? email</label>
              <input
                name="email"
                value={profileData.email}
                readOnly
                className={cn(inputClassName, "cursor-not-allowed bg-cream-dark/10 opacity-70")}
              />
            </div>
            <div>
              <label className={labelClassName}>�?a ch?</label>
              <input
                name="address"
                value={profileData.address}
                onChange={handleProfileChange}
                className={inputClassName}
                placeholder="V� d?: Qu?n 3, TP. H? Ch� Minh"
              />
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center justify-end gap-6 border-t border-cream-dark/50 pt-8">
          ...
          <button
            type="submit"
            disabled={isUpdating}
            className="flex items-center gap-2 rounded-xl bg-[#1A1A1A] px-8 py-3 text-sm font-medium text-[#F9F9F7] shadow-lg shadow-black/10 transition-colors hover:bg-black disabled:opacity-70"
          >
            {isUpdating ? (
              <>
                <Loader2 className="size-4 animate-spin" /> �ang luu...
              </>
            ) : (
              "Luu thay d?i"
            )}
      ...
      <div className="mt-16 space-y-8 border-t border-cream-dark/50 pt-16">
        <div className="mb-8 space-y-2 text-center">
          ...
          <p className="font-body-sm text-ink-muted">
            C?p nh?t m?t kh?u d? t�i kho?n c?a b?n lu�n an to�n.
          </p>
        </div>
        ...
        <form onSubmit={handleSavePassword} className="space-y-6">
          <div>
            ...
            <div className="relative">
              ...
              <button
                ...
                className="absolute right-3 top-1/2 -translate-y-1/2 text-ink-muted transition-colors hover:text-ink focus:outline-none"
                aria-label={showOldPassword ? "?n m?t kh?u hi?n t?i" : "Hi?n m?t kh?u hi?n t?i"}
              >
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <div>
              ...
              <div className="relative">
                ...
                <button
                  ...
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-ink-muted transition-colors hover:text-ink focus:outline-none"
                  aria-label={showNewPassword ? "?n m?t kh?u m?i" : "Hi?n m?t kh?u m?i"}
                >
            ...
            <div>
              <label className={labelClassName}>X�c nh?n m?t kh?u m?i</label>
              <div className="relative">
                <input
                ...
                <button
                  ...
                  className="absolute right-3 top-1/2 -translate-y-1/2 text-ink-muted transition-colors hover:text-ink focus:outline-none"
                  aria-label={
                    showConfirmPassword ? "?n x�c nh?n m?t kh?u" : "Hi?n x�c nh?n m?t kh?u"
                  }
                >
          ...
          <div className="flex items-center space-x-3 pt-2">
            ...
            <label
              ...
              className="cursor-pointer select-none text-[13px] font-medium text-ink-muted"
            >
              �ang xu?t kh?i c�c thi?t b? kh�c
            </label>
          </div>
          ...
          <div className="flex items-center justify-end gap-6 border-t border-cream-dark/50 pt-8">
            ...
            >
              {isChangingPassword ? (
                <>
                  <Loader2 className="size-4 animate-spin" /> �ang c?p nh?t...
                </>
              ) : (
                "C?p nh?t m?t kh?u"
              )}
