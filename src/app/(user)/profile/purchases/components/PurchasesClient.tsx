...
import { useGetActiveBrandDetail, useGetBrandItemDetail } from '@/features/brands/queries/user-brands.queries';
...
function OrderBrandHeader({ brandId, order }: { brandId: string, order: any }) {
  const { data: brand } = useGetActiveBrandDetail(brandId);
  return (
...
function OrderProductItem({ item, order, returnReq }: { item: any, order: any, returnReq: any }) {
  const { data: product } = useGetBrandItemDetail(item.productId);

  return (
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 pb-6 border-b border-border last:border-0 last:pb-0">
      <div className="flex gap-4">
        ...
        <div className="flex flex-col justify-center gap-1">
          <span className="font-bold text-sm">{product?.name || "Đang tải..."}</span>
          ...
          <div className="flex flex-col gap-8">
            {orders.map(order => (
              <div key={order.id} className="border border-border rounded-3xl overflow-hidden flex flex-col">
                {/* Order Header */}
                <OrderBrandHeader brandId={order.brandId} order={order} />

                {/* Order Items */}
                <div className="p-4 sm:p-6 flex flex-col gap-6">
                  {order.items.map((item, idx) => {
                    const returnReq = returnRequests.find(r => r.orderId === order.id && r.productId === item.productId);
                    return <OrderProductItem key={idx} item={item} order={order} returnReq={returnReq} />;
                  })}
                </div>
              </div>
            ))}
