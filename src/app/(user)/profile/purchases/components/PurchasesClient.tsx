...
export default function PurchasesClient() {
  ...
  const getOrderStatusDisplay = (status: string) => {
    switch (status) {
      case "PENDING":
        return <span className="text-orange-600 font-bold text-xs">Chờ xử lý</span>;
      case "DELIVERED":
        return <span className="text-green-600 font-bold text-xs">Đã giao hàng</span>;
      default:
        return <span className="text-black/50 font-bold text-xs">{status}</span>;
    }
  };
  ...
  return (
    <div className="flex-1 bg-white text-black min-h-screen pb-24">
      ...
      <div className="max-w-[1000px] mx-auto px-4 sm:px-6 lg:px-8 py-8 lg:py-12">
        {" "}
        <h1 className="text-3xl lg:text-4xl font-bold tracking-tight mb-10">
          Đơn hàng của tôi
        </h1>{" "}
        {orders.length === 0 ? (
          <div className="text-center py-20 bg-[#FAFAFA] border border-black/10">
            ...
            <Link href="/community">
              {" "}
              <Button className="rounded-xl bg-black text-white font-bold ">
                Tiếp tục mua sắm
              </Button>{" "}
            </Link>{" "}
          </div>
                  ...
                  <div className="p-4 sm:p-6 flex flex-col gap-6">
                    ...
                    {order.items.map((item, idx) => {
                      ...
                      return (
                        <div
                          key={idx}
                          className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 pb-6 border-b border-black/5 last:border-0 last:pb-0"
                        >
                          {" "}
                          <div className="flex gap-4">
                            ...
                            <div className="flex flex-col justify-center gap-1">
                              ...
                              <div className="text-xs text-black/60 mt-1">
                                ...
                                Màu: <span className="font-bold text-black">{item.color}</span>{" "}
                              </div>{" "}
                          ...
                          <div className="flex flex-col sm:items-end gap-3 w-full sm:w-auto">
                            ...
                            {returnReq ? (
                              <div className="bg-[#F5F2EE] px-4 py-2 flex flex-col gap-1 items-start sm:items-end w-full sm:w-auto">
                                ...
                                <span className="text-sm font-bold text-orange-600">
                                  {getReturnStatusDisplay(returnReq.status)}
                                </span>{" "}
                              </div>
                            ) : (
                              <Link
                                href={`/returns/request/${order.id}?productId=${item.productId}`}
                                className="w-full sm:w-auto"
                              >
                                {" "}
                                <Button
                                  variant="outline"
                                  className="w-full sm:w-auto rounded-xl border-black hover:bg-black hover:text-white font-bold text-xs transition-colors"
                                >
                                  {" "}
                                  Yêu cầu Đổi/Trả{" "}
                                </Button>{" "}
                            ...
                            <Button
                              variant="outline"
                              className="w-full sm:w-auto rounded-xl border-black/20 hover:bg-black hover:text-white font-bold text-xs transition-colors"
                            >
                              {" "}
                              Thêm vào Tủ đồ{" "}
                            </Button>{" "}
