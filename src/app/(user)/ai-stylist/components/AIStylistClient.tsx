...
import { Sparkles, Save, RefreshCw, MoveRight, ZoomOut, ZoomIn, MoveUp, RefreshCcw, Layers, SlidersHorizontal, History, X, MessageSquare } from "lucide-react";
...
// Hàm tiện ích để format tin nhắn từ AI: Xử lý xuống dòng, bỏ kí tự đặc biệt thừa
  ...
  return (
    <div className="flex flex-col gap-2">
      {lines.map((line, idx) => {
        ...
        // Nếu là dạng danh sách (bullet point)
        if (/^[*-]\s+/.test(trimmed)) {
          return (
    ...
    try {
      ...
      const formattedMessages: ChatMessage[] = res.items.map((msg, index) => ({
        ...
        if (a.timestamp !== b.timestamp) {
          return a.timestamp - b.timestamp; // Ascending time (oldest first)
        }
        // If timestamps are exactly equal, break tie assuming backend returns DESC (newest first).
        // Larger index means older message, so it should come first.
          ...
          <div className="lg:col-span-4 h-[600px] lg:h-[800px] border border-[#E5E5E5] bg-white flex flex-col relative shadow-sm overflow-hidden">
            ...
            <div className="flex-1 p-5 md:p-6 overflow-y-auto space-y-6 flex flex-col bg-white">
              ...
              {chatMessages.map((msg) => {
                const hasRedirectAction = msg.role === 'ai' && msg.content.includes('[ACTION:REDIRECT_OUTFIT]');
                const cleanContent = msg.content.replace(/\[ACTION:REDIRECT_OUTFIT\]/g, '');
                  ...
                  <div key={msg.id} className={cn("chat-intro flex flex-col gap-1.5 w-full", msg.role === 'user' ? "items-end" : "items-start")}>
                    {msg.role === 'ai' && (
                      <div className="flex items-center gap-2">
                        <span className="text-[9px] font-bold uppercase tracking-widest text-[#A3A3A3]">CLOSY AI</span>
                        <span className="text-[9px] text-[#A3A3A3]/70">{new Date(msg.timestamp).toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' })}</span>
                      </div>
                    )}
                    {msg.role === 'user' && (
                      <div className="flex items-center gap-2">
                        <span className="text-[9px] text-[#A3A3A3]/70">{new Date(msg.timestamp).toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' })}</span>
                        <span className="text-[9px] font-bold uppercase tracking-widest text-[#A3A3A3]">YOU</span>
                      </div>
                    )}
                    ...
                    <div className={cn(
                      ...
                      {msg.role === 'ai' ? formatAIResponse(cleanContent) : cleanContent}
                    </div>
                        ...
                        <button
                          onClick={() => {
                            const userContext = chatMessages
                              ...
                              .map(m => m.content)
                              .join('. ');
                            handleGenerate(userContext);
                          }}
                          disabled={isGenerating || isChatting}
                          className="bg-[#1A1A1A] text-white text-[11px] font-bold py-3 px-6 hover:bg-black/80 transition-colors flex justify-center items-center gap-2 disabled:opacity-50 tracking-widest uppercase shadow-sm border border-[#1A1A1A]"
                        >
                          <Sparkles className="w-3.5 h-3.5" /> TẠO BỘ PHỐI ĐỒ NGAY
                        </button>
                      </div>
                    )}
                  </div>
