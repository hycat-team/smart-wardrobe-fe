  ...
  const handleGenerate = async () => {
    try {
      ...
      const res = await aiApi.getOutfitRecommendation({
        ...
        includeBrandItems: ghostClosetEnabled ? false : undefined,
      });
  ...
  const handleSaveOutfit = async () => {
    ...
    try {
      ...
      if (invalidItems.length > 0) {
        toast.error(
          'Một số trang phục không hợp lệ hoặc thiếu thông tin sản phẩm thời trang. Vui lòng chọn lại!'
        );
        return;
      }
                ...
                <OutfitCanvasBoard
                  ...
                  hasAlternativesCheck={(role) => {
                    ...
                    return !!(
                      ...
                      outfitItem.alternatives.length > 0
                    );
                  }}
                    ...
                    if (outfitData) {
                      ...
                      if (outfitItem) {
                        ...
                        const realGhostItem = allOptions.find(
                          (i) => (i.fashionItem?.id || i.id) === item.clothingItemId
                        );
              ...
              <div className="absolute inset-0 flex items-center justify-center p-8 overflow-hidden">
                {isGenerating ? (
                  <div className="flex flex-col items-center justify-center gap-6">
                    ...
                    <p className="text-foreground font-bold text-sm uppercase tracking-widest">
                      ĐANG TẠO PHONG CÁCH CỦA BẠN...
                    </p>
                  </div>
                ) : (
                  <div className="w-full h-full flex flex-col items-center justify-center text-center max-w-md">
                    ...
                    <h3 className="text-xl font-bold text-foreground uppercase tracking-tight mb-3">
                      Khung sáng tạo
                    </h3>
                    <p className="text-balance text-muted-foreground text-[13px] leading-relaxed">
                      Thiết lập thông số kỹ thuật và nhập ý tưởng thiết kế ở bảng điều khiển bên
                      phải. Hệ thống CLOSY AI sẽ kiến tạo một bộ trang phục dành riêng cho bạn.
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="flex-1 overflow-y-auto p-5 md:p-6 space-y-6">
              <div className="group flex flex-col gap-2 border-b border-border pb-4 focus-within:border-foreground transition-colors">
                ...
                <div className="flex flex-wrap gap-1.5 mt-1">
                  <span className="text-[9px] text-muted-foreground uppercase tracking-widest mr-1 self-center">
                    Gợi ý:
                  </span>
                  {OCCASIONS.map((occ) => (
                    <button
              ...
              {/* <div className="flex items-center justify-between border border-border bg-muted p-4 mt-2 rounded-2xl">
                <div>
                  ...
                  <p className="text-[9px] text-muted-foreground mt-1 tracking-widest uppercase">
                    Thử nghiệm các mẫu thiết kế ảo
                  </p>
                </div>
