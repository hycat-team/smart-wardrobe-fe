      ...
      <div className="max-w-[1200px] mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative -mt-24 sm:-mt-32 mb-12 flex flex-col lg:flex-row items-center lg:items-end gap-6 lg:gap-8">
          <Avatar className="w-32 h-32 sm:w-40 sm:h-40 lg:w-48 lg:h-48 shadow-xl rounded-full overflow-hidden border-4 border-white z-10 shrink-0">
            ...
            <AvatarFallback className="bg-[#111] text-white text-4xl font-semibold">{brand.name[0]}</AvatarFallback>
          </Avatar>
          ...
          <div className="flex-1 flex flex-col items-center lg:items-start gap-4 pb-2 text-center lg:text-left w-full mt-2 lg:mt-0">
            <div className="flex flex-col gap-2 w-full">
              <div className="flex items-center justify-center lg:justify-start gap-3">
                <h1 className="text-3xl sm:text-4xl lg:text-5xl font-semibold font-medium text-[#111] tracking-tight whitespace-nowrap overflow-hidden text-ellipsis">{brand.name}</h1>
                {brand.isVerified && <BadgeCheck className="w-6 h-6 lg:w-8 lg:h-8 text-blue-500 bg-white rounded-full p-0.5" />}
              </div>
              <p className="text-[#666] font-semibold text-sm sm:text-base max-w-[800px] leading-relaxed mx-auto lg:mx-0">
                {brand.description}
              </p>
            </div>
          ...
          <div className="flex flex-col sm:flex-row items-center gap-4 pb-2 w-full lg:w-auto shrink-0 mt-4 lg:mt-0">
            <div className="flex items-center justify-center gap-8 px-6 lg:px-8 py-3 bg-[#FAFAFA] rounded-full border border-black/5 shadow-sm w-full sm:w-auto">
              <div className="flex flex-col items-center">
                <span className="font-bold text-[#111] text-lg font-semibold">{brand.followerCount.toLocaleString()}</span>
                <span className="text-[#A3A3A3] text-[10px] uppercase tracking-widest">Followers</span>
              </div>
              <div className="w-px h-8 bg-black/10"></div>
              <div className="flex flex-col items-center">
                <span className="font-bold text-[#111] text-lg font-semibold">{brand.memberCount.toLocaleString()}</span>
            ...
            <div className="flex gap-3 w-full sm:w-auto">
              <Button
                ...
                className={`flex-1 sm:flex-none min-w-[140px] rounded-full font-semibold text-xs font-medium uppercase tracking-widest px-6 lg:px-8 h-12 shadow-sm transition-colors ${isFollowing ? 'border-black/20 text-black/70 hover:bg-black/5 hover:text-black' : 'bg-black text-white hover:bg-black/90'
                  }`}
              >
        ...
        <div className="mb-16 grid grid-cols-1 lg:grid-cols-3 gap-12">
          <div className="lg:col-span-2 flex flex-col gap-4">
            <h3 className="font-semibold text-2xl font-medium text-[#111] border-b border-black/10 pb-4">Câu chuyện thương hiệu</h3>
            <p className="text-[#666] leading-relaxed font-semibold text-sm">{brand.story}</p>
          </div>
          <div className="flex flex-col gap-4">
            <h3 className="font-semibold text-2xl font-medium text-[#111] border-b border-black/10 pb-4">Phong cách</h3>
            <div className="flex flex-wrap gap-2">
              {brand.styles?.map((style: string) => (
                <span key={style} className="px-4 py-2 bg-[#FAFAFA] border border-black/5 rounded-full text-xs font-medium uppercase tracking-widest text-[#111] font-semibold hover:bg-black hover:text-white transition-colors cursor-pointer">
                  {style}
                </span>
              ))}
            </div>
        ...
        <Tabs defaultValue="shop" className="w-full">
          <TabsList variant="line" className="w-full justify-start h-auto p-0 border-b border-black/10 mb-8 lg:mb-12 gap-6 lg:gap-8 overflow-x-auto flex-nowrap [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            <TabsTrigger
              value="shop"
              className="px-0 pb-4 pt-2 font-semibold text-sm font-bold uppercase tracking-widest text-[#A3A3A3] data-[state=active]:text-[#111] bg-transparent data-[state=active]:bg-transparent"
            >
              Cửa hàng
            </TabsTrigger>
            <TabsTrigger
          ...
          <TabsContent value="shop" className="focus-visible:outline-none focus-visible:ring-0 mt-0 animate-in fade-in slide-in-from-bottom-4 duration-700">
            {products.length === 0 ? (
              <div className="text-center py-20 text-[#A3A3A3] font-semibold text-sm uppercase tracking-widest">Brand chưa đăng sản phẩm nào.</div>
            ) : (
                ...
                {products.map(product => (
                  <Link key={product.id} href={`/products/${product.id}`} className="group flex flex-col gap-4">
                    ...
                    <div className="flex flex-col gap-2">
                      <h4 className="font-semibold text-lg font-medium text-[#111] group-hover:underline leading-tight line-clamp-1">{product.name}</h4>
          ...
          <TabsContent value="posts" className="focus-visible:outline-none focus-visible:ring-0 mt-0 animate-in fade-in slide-in-from-bottom-4 duration-700">
            {brandPosts.length === 0 ? (
              <div className="text-center py-20 text-[#A3A3A3] font-semibold text-sm uppercase tracking-widest">Brand chưa có bài đăng nào.</div>
            ) : (
                    ...
                    <div className="p-5 flex items-center justify-between border-b border-black/5 bg-[#FAFAFA]/50">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex flex-col">
                          <span className="font-bold text-sm text-[#111] font-semibold">{brand.name}</span>
