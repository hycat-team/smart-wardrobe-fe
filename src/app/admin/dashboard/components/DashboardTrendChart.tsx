    ...
    return (
      <section className="rounded-3xl border border-dashed border-border bg-card p-12 text-center">
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Hãy chọn khoảng ngày khác để xem dữ liệu đã tổng hợp.
        </p>
      </section>
    );

  const visibleMetrics: AdminMetricKey[] =
    tab === 'finance'
    ...
    >
      <div className="flex flex-col gap-5 border-b border-border pb-5 sm:flex-row sm:items-end sm:justify-between">
        ...
        <Tabs value={tab} onValueChange={(value) => setTab(value as TrendTab)}>
          <TabsList aria-label="Nhóm dữ liệu biểu đồ">
            ...
            <TabsTrigger value="ai">AI Health</TabsTrigger>
          </TabsList>
        </Tabs>
      </div>
      ...
      <div className="mt-6 h-[300px] w-full">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart
            data={chartData}
            margin={{ top: 8, right: hasRateAxis ? 8 : 12, left: 4, bottom: 0 }}
          >
            ...
            <Tooltip
              ...
              formatter={(value, name) =>
                formatMetric(String(name) as AdminMetricKey, Number(value))
              }
            />
        ...
        {visibleMetrics.map((metric) => (
          <div key={metric} className="rounded-2xl bg-muted/50 px-4 py-3">
            ...
            <p className="mt-2 font-semibold tabular-nums">
              {formatMetric(metric, Number(latest?.[metric] ?? 0))}
            </p>
          </div>
        ))}
      </div>
