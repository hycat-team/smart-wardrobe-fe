        ...
        <Table>
          <TableHeader>
            <TableRow className="hover:bg-transparent border-b border-black/10">
              <TableHead className="font-semibold text-[10px] text-[#666] uppercase tracking-[0.15em] py-4 h-auto w-[350px]">Bài viết</TableHead>
              <TableHead className="font-semibold text-[10px] text-[#666] uppercase tracking-[0.15em] py-4 h-auto">Người đăng</TableHead>
              <TableHead className="font-semibold text-[10px] text-[#666] uppercase tracking-[0.15em] py-4 h-auto text-center">Tương tác</TableHead>
              <TableHead className="font-semibold text-[10px] text-[#666] uppercase tracking-[0.15em] py-4 h-auto">Trạng thái</TableHead>
              <TableHead className="font-semibold text-[10px] text-[#666] uppercase tracking-[0.15em] py-4 h-auto text-right">Thao tác</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {isLoading ? (
              ...
              return (
                <TableRow key={trend.id || trend.publicId} className={cn("border-b border-black/5 hover:bg-[#F8F7F5] transition-colors", isHidden && "opacity-60")}>
                  <TableCell className="py-5">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="flex flex-col">
                        <span className="font-semibold font-medium text-base text-[#111] line-clamp-2">{title}</span>
                        <span className="font-semibold text-[10px] text-[#666] uppercase tracking-widest mt-1">{date}</span>
                      </div>
                    </div>
                  </TableCell>
                  <TableCell className="py-5 font-semibold text-[12px] text-[#111]">@{creator}</TableCell>
                  <TableCell className="py-5">
                    <div className="flex items-center justify-center gap-4 font-semibold text-[11px] text-[#666]">
                  ...
                  <TableCell className="py-5">
                    <span className={cn("font-semibold text-[10px] uppercase tracking-widest font-medium border px-2 py-1", !isHidden ? "text-[#111] border-[#111]" : "text-[#A3A3A3] border-black/10")}>
                      {!isHidden ? "ACTIVE" : "DELETED"}
                    </span>
                  </TableCell>
                  <TableCell className="text-right py-5">
                    <Button variant="ghost" size="sm" onClick={() => setSelectedTrend({ ...trend, title, creator, date, imageUrl })} className="text-[#A3A3A3] hover:text-[#111] hover:bg-transparent rounded-none font-semibold text-[10px] uppercase tracking-widest">
          ...
          {selectedTrend && (
            <div className="flex flex-col">
              ...
              <div className="p-8 border-t border-black/10 space-y-6">
                <div>
                  <h3 className="font-semibold text-3xl font-medium text-[#111] line-clamp-3 leading-snug">{selectedTrend.title}</h3>
                  <p className="font-semibold text-[10px] uppercase tracking-widest text-[#666] mt-4">
                ...
                <div className="flex gap-8 border-t border-black/10 pt-6">
                  <div className="space-y-1">
                    <p className="font-semibold text-[10px] uppercase tracking-widest text-[#A3A3A3]">LƯỢT THÍCH</p>
                    <p className="font-semibold text-lg text-[#111]">{selectedTrend.likeCount || 0}</p>
                  </div>
                  <div className="space-y-1">
                    <p className="font-semibold text-[10px] uppercase tracking-widest text-[#A3A3A3]">BÌNH LUẬN</p>
                    <p className="font-semibold text-lg text-[#111]">{selectedTrend.commentCount || 0}</p>
                  </div>
                </div>
              </div>
            </div>
