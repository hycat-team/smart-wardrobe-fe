        ...
        <Table>
          <TableHeader>
            <TableRow className="hover:bg-transparent border-b border-black/10">
              <TableHead className="font-semibold text-[10px] text-[#666] uppercase tracking-[0.15em] py-4 h-auto w-[300px]">Tài khoản</TableHead>
              <TableHead className="font-semibold text-[10px] text-[#666] uppercase tracking-[0.15em] py-4 h-auto">Tên đăng nhập</TableHead>
              <TableHead className="font-semibold text-[10px] text-[#666] uppercase tracking-[0.15em] py-4 h-auto">Ngày tham gia</TableHead>
              <TableHead className="font-semibold text-[10px] text-[#666] uppercase tracking-[0.15em] py-4 h-auto">Trạng thái</TableHead>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow><TableCell colSpan={5} className="h-32 text-center text-[#111] font-semibold uppercase text-[11px] border-b border-black/5">Lỗi tải dữ liệu.</TableCell></TableRow>
            ) : users.length === 0 ? (
              <TableRow><TableCell colSpan={5} className="h-32 text-center text-[#A3A3A3] font-semibold uppercase text-[11px] tracking-widest border-b border-black/5">Không có dữ liệu.</TableCell></TableRow>
            ) : users.map((user: any) => {
                ...
                <TableRow key={user.id} className={cn("border-b border-black/5 hover:bg-[#F8F7F5] transition-colors", !isActive && "opacity-60")}>
                  <TableCell className="py-5">
                    <div className="flex items-center gap-4">
                      <div className="size-10 bg-black/5 flex items-center justify-center font-semibold text-lg text-[#111] overflow-hidden shrink-0">
                        {user.avatarUrl ? (
                      ...
                      <div className="flex flex-col overflow-hidden">
                        <span className="font-semibold font-medium text-base text-[#111] truncate">{displayName}</span>
                        <span className="font-semibold text-[10px] text-[#666] truncate uppercase tracking-wider mt-0.5">{user.email}</span>
                      </div>
                    </div>
                  </TableCell>
                  <TableCell className="font-semibold text-[12px] text-[#666] py-5">{user.username}</TableCell>
                  <TableCell className="font-semibold text-[12px] text-[#666] py-5">
                    {user.createdAt ? new Date(user.createdAt).toLocaleDateString('vi-VN', { day: '2-digit', month: '2-digit', year: 'numeric' }) : 'N/A'}
                  </TableCell>
                  <TableCell className="py-5">
                    <span className={cn("font-semibold text-[10px] uppercase tracking-widest font-medium border px-2 py-1", isActive ? "text-[#111] border-[#111]" : "text-[#A3A3A3] border-black/10")}>
                  ...
                  <TableCell className="text-right py-5">
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end" className="w-48 rounded-none border border-black/10 bg-white p-2 shadow-sm font-semibold uppercase tracking-widest text-[10px]">
          ...
          {selectedUser && (
            <div className="flex flex-col">
              <div className="p-8 border-b border-black/10 flex flex-col items-center gap-6">
                <div className="size-24 bg-black/5 flex items-center justify-center font-semibold text-3xl text-[#111]">
                ...
                <div className="text-center space-y-1">
                  <h3 className="font-semibold text-3xl font-medium text-[#111]">
                    {`${selectedUser.firstName || ''} ${selectedUser.lastName || ''}`.trim() || selectedUser.username || 'Người dùng'}
                  </h3>
                  <p className="font-semibold text-[11px] uppercase tracking-widest text-[#666]">{selectedUser.email}</p>
                </div>
              </div>

              <div className="p-8 grid grid-cols-2 gap-8">
                <div className="space-y-2">
                  <p className="font-semibold text-[10px] uppercase tracking-widest text-[#A3A3A3]">USERNAME</p>
                  <p className="font-semibold text-[12px] text-[#111]">{selectedUser.username || 'N/A'}</p>
                </div>
                <div className="space-y-2">
                  <p className="font-semibold text-[10px] uppercase tracking-widest text-[#A3A3A3]">ĐIỆN THOẠI</p>
                  <p className="font-semibold text-[12px] text-[#111]">{selectedUser.phone || 'N/A'}</p>
                </div>
                <div className="space-y-2">
                  <p className="font-semibold text-[10px] uppercase tracking-widest text-[#A3A3A3]">NGÀY THAM GIA</p>
                  <p className="font-semibold text-[12px] text-[#111]">
                ...
                <div className="space-y-2">
                  <p className="font-semibold text-[10px] uppercase tracking-widest text-[#A3A3A3]">TRẠNG THÁI</p>
                  <span className={cn("inline-block font-semibold text-[10px] uppercase tracking-widest font-medium border px-2 py-1 mt-1", (selectedUser.status === 0 || selectedUser.status === "ACTIVE") ? "text-[#111] border-[#111]" : "text-[#A3A3A3] border-black/10")}>
