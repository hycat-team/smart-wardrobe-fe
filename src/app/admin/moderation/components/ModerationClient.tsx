...
export function ModerationClient() {
  ...
  return (
    <div className="flex flex-col gap-10 animate-in fade-in duration-500 max-w-[1400px] mx-auto w-full pb-24 text-[#111]">
      {" "}
      {/* High-end Chỉnh sửaorial Header */}{" "}
      ...
      <Tabs defaultValue="posts" className="w-full">
        {" "}
        <TabsList className="mb-8 w-full justify-start border-b border-black/10 rounded-xl bg-transparent h-auto p-0 gap-8">
          {" "}
          <TabsTrigger
            value="posts"
            className="rounded-xl border-b-2 border-transparent data-[state=active]:border-[#111] data-[state=active]:bg-transparent data-[state=active]:shadow-none px-0 py-3 font-sans text-[11px] text-[#A3A3A3] data-[state=active]:text-[#111]"
          >
            {" "}
            FEED POSTS{" "}
          </TabsTrigger>{" "}
...
function PostsModerationList({ searchTerm }: { searchTerm: string }) {
  ...
  return (
    <div className="space-y-0 border border-black/10 bg-white">
        ...
        posts.map((post: any) => {
          ...
          return (
            <div
              ...
              className={cn(
                "p-6 border-b border-black/5 flex flex-col md:flex-row gap-6 hover:bg-[#F8F7F5] transition-colors",
            ...
            >
              ...
              <div className="flex gap-6 md:w-[40%] shrink-0">
                ...
                <div className="flex flex-col justify-center gap-2">
                  {" "}
                  <h4 className="font-sans font-medium text-xl text-[#111] line-clamp-2 leading-snug">
                    {post.content || post.title || "Không có nội dung"}
                  </h4>{" "}
                  <p className="font-sans text-[10px] text-[#666]">BY @{authorName}</p>{" "}
                </div>{" "}
              </div>{" "}
              ...
              <div className="flex-1 flex flex-col md:flex-row justify-between gap-6">
                ...
                <div className="space-y-3 flex-1 flex flex-col justify-center">
                  {" "}
                  <div className="flex gap-4 font-sans text-[10px] text-[#666]">
                    {" "}
                    <span>
                      {post.createdAt
                    ...
                    <span
                      className={cn("font-medium", isDeleted ? "text-[#A3A3A3]" : "text-[#111]")}
                    >
                      {isDeleted ? "DELETED" : "ACTIVE"}
                    </span>{" "}
                  </div>{" "}
                  <div className="flex items-center gap-6 text-[#111] font-sans text-[11px]">
                    {" "}
                    <span className="flex items-center gap-1.5">
                      <Heart className="size-3" strokeWidth={1.5} /> {post.likeCount || 0}
                    </span>{" "}
                    <span className="flex items-center gap-1.5">
                ...
                <div className="flex flex-row md:flex-col justify-end gap-3 shrink-0 border-t md:border-t-0 pt-4 md:pt-0 border-black/10">
                  ...
                  <Button
                    variant="outline"
                    onClick={() => setViewingCommentsPostId(post.publicId || post.id)}
                    className="rounded-xl border-black/10 font-sans text-[10px] hover:bg-black/5 h-8"
                  >
                    {" "}
                    COMMENTS{" "}
                  </Button>{" "}
                  {isDeleted ? (
                    <Button
                      ...
                      variant="outline"
                      className="rounded-xl border-[#111] text-[#111] font-sans text-[10px] hover:bg-[#111] hover:text-white h-8"
                    >
                      {" "}
                      RESTORE{" "}
                    </Button>
                    ...
                    <Button
                      disabled={isBusy}
                      onClick={() => deletePost(post.publicId || post.id)}
                      variant="outline"
                      className="rounded-xl border-black/10 text-red-600 font-sans text-[10px] hover:bg-red-50 hover:border-red-600 h-8"
                    >
                      {" "}
                      DELETE{" "}
                    </Button>
      ...
      />{" "}
      <Dialog
        open={!!viewingCommentsPostId}
        onOpenChange={(open) => !open && setViewingCommentsPostId(null)}
      >
        {" "}
        <DialogContent className="sm:max-w-[600px] max-h-[80vh] overflow-y-auto rounded-xl border border-black/10 bg-white p-8">
          {" "}
          <DialogHeader className="border-b border-black/10 pb-4 mb-6">
            {" "}
            <DialogTitle className="font-sans text-3xl font-medium text-[#111]">
              Comments
            </DialogTitle>{" "}
          </DialogHeader>{" "}
  ...
  if (isLoading)
    return (
      <div className="py-8 text-center">
        <Loader2 className="animate-spin size-5 mx-auto text-[#A3A3A3]" />
      </div>
    );
  if (isError)
    return (
      <div className="py-8 text-center text-[#111] font-sans text-[11px] ">
        Lỗi khi tải bình luận.
      </div>
    );
  if (!comments || comments.length === 0)
    return (
      <div className="py-8 text-center text-[#A3A3A3] font-sans text-[11px] border border-black/10">
        Không có bình luận.
      </div>
    );
  return (
      ...
      {comments.map((comment: any, index: number) => {
        ...
        return (
          <div
            key={comment.id}
            className={cn(
              "p-6 flex gap-4 bg-white",
              index !== comments.length - 1 && "border-b border-black/10",
              isDeleted && "opacity-60"
            )}
          >
            {" "}
            <div className="size-8 bg-black/5 shrink-0 flex items-center justify-center font-sans text-lg text-[#111]">
              {" "}
              {comment.avatarUrl ? (
                <img
              ...
              <div className="flex justify-between items-start">
                ...
                {isDeleted ? (
                  <button
                    ...
                    className="font-sans text-[10px] text-[#111] hover:underline underline-offset-4"
                  >
                    RESTORE
                  </button>
                ) : (
                  <button
                    ...
                    disabled={isDeleting}
                    className="font-sans text-[10px] text-red-600 hover:underline underline-offset-4"
                  >
                    DELETE
                  </button>
                )}{" "}
              </div>{" "}
              <p className="text-sm font-sans text-[#111] leading-relaxed">
                {comment.content}
              </p>{" "}
            </div>{" "}
          </div>
    ...
    <div className="space-y-0 border border-black/10 bg-white">
      ...
      {isLoading ? (
        ...
        items.map((item: any) => {
          ...
          const imageUrl =
            item.imageUrl || item.images?.[0] || "https://placehold.co/200x200?text=No+Image";
            ...
            >
              ...
              <div className="flex gap-6 md:w-[40%] shrink-0">
                ...
                <div className="flex flex-col justify-center gap-2">
                  {" "}
                  <h4 className="font-sans font-medium text-xl text-[#111] line-clamp-2 leading-snug">
                    {item.name || item.title || "Sản phẩm không có tên"}
                  </h4>{" "}
                  <p className="font-sans text-[10px] text-[#666]">SELLER @{sellerName}</p>{" "}
                </div>{" "}
              </div>{" "}
              <div className="hidden md:block w-px bg-black/10 my-2" />{" "}
              <div className="flex-1 flex flex-col md:flex-row justify-between gap-6">
                ...
                <div className="space-y-3 flex-1 flex flex-col justify-center">
                  {" "}
                  <p className="font-sans text-lg font-medium text-[#111]">
                    {item.price ? `${item.price.toLocaleString("vi-VN")} VNĐ` : "THỎA THUẬN"}
                  </p>{" "}
                  <p
                    className={cn(
                      "font-sans text-[10px] ",
                      isHidden ? "text-[#A3A3A3]" : "text-[#111]"
                    )}
                  >
                ...
                <div className="flex flex-row md:flex-col justify-end gap-3 shrink-0 border-t md:border-t-0 pt-4 md:pt-0 border-black/10">
                  ...
                  {!isHidden && (
                    <Button
                      ...
                      variant="outline"
                      className="rounded-xl border-black/10 text-[#666] font-sans text-[10px] hover:bg-black/5 h-8"
                    >
                      {" "}
                      HIDE ITEM{" "}
                    </Button>
...
function LoadingState() {
  return (
    <div className="py-24 flex justify-center">
      <Loader2 className="animate-spin text-[#A3A3A3] size-6" />
    </div>
  );
}
function ErrorState() {
  return (
    <div className="py-24 text-center text-[#111] font-sans text-[11px] ">Lỗi khi tải dữ liệu</div>
  );
}
function EmptyState() {
  return (
    <div className="py-24 text-center text-[#A3A3A3] font-sans text-[11px] ">
    ...
    <div className="flex justify-between items-center p-6 border-t border-black/10 bg-white">
      ...
      <div className="flex gap-2">
        ...
        <Button
          variant="outline"
          size="sm"
          className="rounded-xl border-black/10 font-sans text-[10px] text-[#111] hover:bg-black/5"
          disabled={page <= 1}
