    ...
    <div className="flex flex-col gap-10 animate-in fade-in duration-500 max-w-[1400px] mx-auto w-full pb-24 text-[#111]">
      ...
      <div className="flex flex-col gap-8 pt-6 border-b border-black/10 pb-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
          ...
          <div className="flex gap-4 w-full md:w-auto">
            ...
            <Button onClick={openAdd} className="shrink-0 rounded-none bg-[#111] text-white hover:bg-black font-['IBM_Plex_Mono'] text-[11px] uppercase tracking-widest gap-2">
        ...
        <Table>
          <TableHeader>
            <TableRow className="hover:bg-transparent border-b border-black/10">
              <TableHead className="font-['IBM_Plex_Mono'] text-[10px] text-[#666] uppercase tracking-[0.15em] py-4 h-auto">Tên danh mục</TableHead>
              <TableHead className="font-['IBM_Plex_Mono'] text-[10px] text-[#666] uppercase tracking-[0.15em] py-4 h-auto">Đường dẫn (Slug)</TableHead>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow><TableCell colSpan={3} className="h-32 text-center text-[#111] font-['IBM_Plex_Mono'] uppercase text-[11px] border-b border-black/5">Lỗi tải dữ liệu.</TableCell></TableRow>
            ) : filteredItems.length === 0 ? (
              <TableRow><TableCell colSpan={3} className="h-32 text-center text-[#A3A3A3] font-['IBM_Plex_Mono'] uppercase text-[11px] tracking-widest border-b border-black/5">Không có dữ liệu.</TableCell></TableRow>
            ) : filteredItems.map((item: any) => (
              <TableRow key={item.id} className="border-b border-black/5 hover:bg-[#F8F7F5] transition-colors">
                <TableCell className="font-['Playfair_Display'] text-lg font-medium py-5">{item.name}</TableCell>
                <TableCell className="font-['IBM_Plex_Mono'] text-[12px] text-[#666] py-5">{item.slug}</TableCell>
      ...
      <Dialog open={!!deletingId} onOpenChange={(open) => !open && setDeletingId(null)}>
        <DialogContent className="rounded-none border-black/10 bg-white">
          <DialogHeader>
            <DialogTitle className="font-['Playfair_Display'] text-2xl font-medium text-[#111]">Xác nhận xóa</DialogTitle>
            <DialogDescription className="font-['IBM_Plex_Mono'] text-[11px] uppercase tracking-[0.05em] text-[#666]">
              Bạn có chắc chắn muốn xóa danh mục này? Hành động này không thể hoàn tác.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter className="mt-6">
            <Button variant="outline" onClick={() => setDeletingId(null)} className="rounded-none font-['IBM_Plex_Mono'] text-[11px] uppercase tracking-widest border-black/10">Hủy</Button>
            <Button variant="destructive" onClick={handleDelete} disabled={isDeleting} className="rounded-none font-['IBM_Plex_Mono'] text-[11px] uppercase tracking-widest bg-red-600 hover:bg-red-700">
      ...
      <Dialog open={isAdding || !!editingItem} onOpenChange={(open) => !open && handleCloseDialog()}>
        <DialogContent className="sm:max-w-[425px] rounded-none border-black/10 bg-white">
          <DialogHeader>
            <DialogTitle className="font-['Playfair_Display'] text-2xl font-medium text-[#111]">
          ...
          <form onSubmit={handleSave} className="space-y-6 mt-4">
            <div className="space-y-4">
              <div className="space-y-2">
                <Label className="font-['IBM_Plex_Mono'] text-[10px] uppercase tracking-widest text-[#666]">Tên danh mục</Label>
                <Input 
                  value={formData.name} 
                  onChange={(e) => setFormData({...formData, name: e.target.value})} 
                  placeholder="Áo thun..." 
                  className="rounded-none border-black/10 focus-visible:ring-0 focus-visible:border-[#111] font-['IBM_Plex_Mono'] text-[12px]"
                  required 
                />
              </div>
              <div className="space-y-2">
                <Label className="font-['IBM_Plex_Mono'] text-[10px] uppercase tracking-widest text-[#666]">Đường dẫn (Slug)</Label>
                <Input 
                  value={formData.slug} 
            ...
            <DialogFooter>
              <Button type="button" variant="outline" onClick={handleCloseDialog} className="rounded-none font-['IBM_Plex_Mono'] text-[11px] uppercase tracking-widest border-black/10">Hủy</Button>
              <Button type="submit" disabled={isCreating || isUpdating} className="rounded-none bg-[#111] text-white hover:bg-black font-['IBM_Plex_Mono'] text-[11px] uppercase tracking-widest">
