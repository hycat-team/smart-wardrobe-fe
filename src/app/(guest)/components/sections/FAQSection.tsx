    ...
    >
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="text-center mb-16">
          {" "}
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white border border-[#1A1A1A]/10 text-xs font-bold text-[#D9C5B2] mb-6 shadow-sm">
            {" "}
            Câu hỏi thường gặp{" "}
          </div>{" "}
          <h2
            id="faq-heading"
            className="font-sans text-4xl md:text-6xl text-[#1A1A1A] font-medium"
          >
            {" "}
            Bạn thắc mắc?{" "}
          </h2>{" "}
          ...
          {FAQ_ITEMS.map((item, index) => (
            ...
            >
              ...
              >
                ...
                <ChevronDown
                  className={`size-5 shrink-0 text-[#707070] transition-transform duration-300 ${openIndex === index ? "rotate-180" : ""}`}
                />{" "}
              </button>{" "}
              <div
                id={`faq-answer-${index}`}
                role="region"
                className={`overflow-hidden transition-all duration-300 ease-out ${openIndex === index ? "max-h-48 opacity-100" : "max-h-0 opacity-0"}`}
              >
                {" "}
