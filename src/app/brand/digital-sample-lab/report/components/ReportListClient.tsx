...
export function ReportListClient() {
  ...
  return (
    <div className="max-w-6xl mx-auto py-12 px-6 space-y-10">
      {/* Header */}
      <div>
        <Link href="/brand/digital-sample-lab" className="text-[10px] font-mono uppercase tracking-widest text-ink-muted hover:text-ink flex items-center gap-1 mb-6 w-fit">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-ink/10 pb-6">
          ...
          <Link href="/brand/digital-sample-lab">
            <Button className="bg-ink text-cream hover:bg-ink/90 font-mono text-[11px] uppercase tracking-widest rounded-none flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {reports.length === 0 ? (
          <div className="col-span-full py-12 text-center border border-dashed border-ink/20 bg-[#F4F1EE]/50">
            <p className="text-sm font-mono tracking-widest uppercase text-ink-muted mb-4">Chưa có báo cáo nào</p>
            <Link href="/brand/digital-sample-lab">
              <Button variant="outline" className="font-mono text-[11px] uppercase tracking-widest rounded-none border-ink/20">
                  ...
                  <div className="flex flex-wrap gap-2 mb-6">
                    {report.variants?.map((v: any, i: number) => (
                      <div key={i} className="flex items-center gap-1.5 bg-[#F4F1EE] px-2 py-1">
                        <div className="w-2.5 h-2.5 rounded-full border border-ink/10" style={{ backgroundColor: v.color || '#000' }} />
                        <span className="text-[10px] font-mono uppercase tracking-widest">{v.name}</span>
                      </div>
                    ))}
                  </div>
                </div>
