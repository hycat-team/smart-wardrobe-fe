  ...
  const NavLinks = () => (
    <>
      ...
      <Link
        ...
        className={getLinkClass("/brand/dashboard")}
      >
        {" "}
      ...
      <Link
        href="/brand/products"
        onClick={() => setIsMobileMenuOpen(false)}
        className={getLinkClass("/brand/products")}
      >
        {" "}
      ...
      <Link
        href="/brand/posts"
        onClick={() => setIsMobileMenuOpen(false)}
        className={getLinkClass("/brand/posts")}
      >
        {" "}
        <MessageSquareText className="w-5 h-5" /> Bài viết{" "}
      </Link>{" "}
      <Link
        href="/brand/customer-care"
        onClick={() => setIsMobileMenuOpen(false)}
        className={getLinkClass("/brand/customer-care")}
      >
        {" "}
        <MessageSquareText className="w-5 h-5" /> Chăm sóc khách hàng{" "}
      </Link>{" "}
      <Link
        href="/brand/digital-sample-lab/report"
        onClick={() => setIsMobileMenuOpen(false)}
        className={getLinkClass("/brand/digital-sample-lab/report")}
      >
        {" "}
      ...
      <Link
        href="/brand/profile"
        onClick={() => setIsMobileMenuOpen(false)}
        className={getLinkClass("/brand/profile")}
      >
        {" "}
    ...
    <div className="flex h-screen bg-[#FAFAFA] text-black overflow-hidden relative">
      ...
      <aside className="hidden lg:flex w-64 border-r border-black/10 bg-white flex-col shrink-0">
        ...
        <div className="h-16 flex items-center px-6 border-b border-black/10">
          {" "}
          <span className="font-bold text-lg tracking-tight ">Không gian thương hiệu</span>{" "}
        </div>{" "}
        ...
        <div className="p-4 border-t border-black/10">
          {" "}
          <Link
            href="/brands/brand_001"
            className="flex items-center gap-3 px-4 py-3 rounded-sm hover:bg-black/5 font-bold text-sm transition-colors text-black/50"
          >
            {" "}
      ...
      {/* Mobile Sidebar */}{" "}
      <aside
        className={`fixed inset-y-0 left-0 w-64 bg-white z-50 flex flex-col transform transition-transform duration-300 lg:hidden ${isMobileMenuOpen ? "translate-x-0" : "-translate-x-full"}`}
      >
        {" "}
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <div className="h-16 border-b border-black/10 bg-white flex items-center justify-between px-4 lg:px-8 shrink-0">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="font-bold text-sm text-black/50 hidden sm:block">Mori Studio</div>{" "}
          </div>{" "}
